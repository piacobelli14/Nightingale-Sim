...
struct ResetAuth: View {
    ...
    private func initiatePasswordReset() {
        guard let url = URL(string: "https://www.nightingale-health.org/vektor/vektor-web-api/reset-password") else {
            return
        }
    ...
    private func setNewPassword() {
        ...
        let url = URL(string: "https://www.nightingale-health.org/vektor/vektor-web-api/change-password")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
